<template>
    <section class="mt-20 lg:mt-0 w-full relative text-white flex justify-center">
        ...
        <ul ref="statsSection"
            ...
            <li v-for="element in numbers" :key="element.id" class="text-center">
                <h2 class="font-semibold flex justify-center text-xl sm:text-2xl md:text-4xl w-full">
                    +
                    <Countup v-if="hasIntersected" :end-Val="element.number" />
                </h2>
...
import Countup from 'vue-countup-v3'; // Ensure this import is correct

const numbers = ref([
    { id: 1, number: 12, title: 'Created Projects' },
    { id: 2, number: 200, title: 'Products' },
